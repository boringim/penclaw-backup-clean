...
$tarExcludes = @(
    ...
    "--exclude=.openclaw/experiments/camoufox-wrapper/profiles",
    "--exclude=.openclaw/**/Cache",
    "--exclude=.openclaw/**/Cache_Data",
    "--exclude=.openclaw/**/Code Cache",
    "--exclude=.openclaw/**/*.log",
    "--exclude=.openclaw/**/*.tmp",
    "--exclude=.openclaw/**/*.lock"
)

& tar -czf $backupFile @tarExcludes -C 'C:\Users\Administrator' .openclaw
